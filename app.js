...
new Vue({ // eslint-disable-line no-new
  ...
  methods: {
    selectModule: function (lang, module, line = null) {
    ...
    processTags: function (raw) {
      ...
      raw.split('\n').forEach(line => {
        ...
        if (s[1] === 'indir') {
          ...
          r.language = bits[bits.length - 2]
          const modline = bits[bits.length - 1]
          r.module = modline.slice(0, modline.indexOf('.'))
          r.line = modline.slice(modline.indexOf(':') + 1)
          r.ftype = s[3]
        }
        rs.push(r)
      })
